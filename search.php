...
<div class="title">
  ...
  <div class="searchbar">
    <form action="search.php" method="post">
      <input type="text" placeholder="Search.." name="search" id="searchtext" value="<?php echo $_POST["search"] ?>">
...
$sql = "select * from songs where album like'%".$search."%';";
$result =$conn->query($sql);
    if($result->num_rows > 0)
    {
      while($row = $result->fetch_assoc()) {
...
$sql = "select * from songs where artist_name like'%".$search."%';";
$result =$conn->query($sql);
    if($result->num_rows > 0)
    {
      while($row = $result->fetch_assoc()) {
  ...
  {
    ?>   
      <form class="form" id="played" method="post" action="home.php">
          <div class="nono">
            <label style="width:100%; color: black; background-color: white; padding: 20px; text-align: center; font-size: 20px;">
              Not Found
            </label>
          </div>
      </form> 
